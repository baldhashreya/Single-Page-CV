...
export function PortfolioPage({ data }: { data: CvData }) {
  return (
    <div className="portfolio-wrapper">
      {/* Hero Section */}
      <section className="glass-card section-hero">
        <div className="hero-content">
          <span className="role-badge">{data.hero.role}</span>
          <h1>
          ...
          <div className="contact-links">
            {data.hero.contacts.map((contact) => (
              <a
                key={contact.label}
                href={contact.href}
      ...
      <section className="glass-card section-other">
        <h2 className="section-title">{data.leadership.title}</h2>
